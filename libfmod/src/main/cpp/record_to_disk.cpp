...
#pragma pack(1)
        ...
        */
        ...
        FmtChunk = {{{'f', 'm', 't', ' '}, sizeof(FmtChunk) - sizeof(RiffChunk)}, 1, channels,
                    (int) rate, (int) rate * channels * bits / 8, 1 * channels * bits / 8, bits}
                __PACKED;
        ...
        } WavHeader = {{{'R', 'I', 'F', 'F'}, sizeof(FmtChunk) + sizeof(RiffChunk) + length},
                       {'W',                  'A', 'V', 'E'}};
    ...
    */
    do {
        ...
        if (recordpos != lastrecordpos) {
            ...
            */
            sound->lock(lastrecordpos * exinfo.numchannels * 2,
                        blocklength * exinfo.numchannels * 2, &ptr1, &ptr2, &len1,
                        &len2);   /* * exinfo.numchannels * 2 = stereo 16bit.  1 sample = 4 bytes. */
        ...
        printf("%-23s. Record buffer pos = %6d : Record time = %02d:%02d\r",
               (timeGetTime() / 500) & 1 ? "Recording to record.wav" : "", recordpos,
               datalength / exinfo.defaultfrequency / exinfo.numchannels / 2 / 60,
               (datalength / exinfo.defaultfrequency / exinfo.numchannels / 2) % 60);
